...
export default class StackDriverCapability {
            ...
            error: (context: IopaContext, ex: any, ...optionalArgs) => {
                ...
                if (typeof ex == 'string') {
                    ...
                    if (/UnhandledPromiseRejectionWarning.*at /s.test(ex)) {
                        StackDriver.error(ex, this.app.properties.get("server.Version"), context)
                    } else {
    ...
    async invoke(context: IopaContext, next: () => Promise<void>) {
        ...
        const labels = Array.from(context.get("iopa.Labels")).reduce((obj, [key, value]) => {
            obj[key] = value;
            return obj;
        }, {});
        ...
        StackDriver.log({ logs: body }, context.response.get("iopa.StatusCode") < 300 ? LogEntry.SeverityEnum.INFO : LogEntry.SeverityEnum.WARNING, labels, context, true)
...
class StackDriver {
    ...
    public static log(payload, severity: LogEntry.SeverityEnum, labels, context: IopaContext, isRequest: boolean = false) {
        ...
        const logEntry: LogEntry = {
            ...
            severity,
            labels,
            trace: `projects/${process.env.FIREBASE_PROJECT_ID}/traces/${context.get("server.Id")}`,
            ...
            resource: {
                "labels": {
                    ...
                    "function_name": context.get("iopa.Path").replace(/^\//, '').replace(/\//g, '-'),
    ...
    public static async error(ex: Error | string, version: string, context: IopaContext) {
        ...
        const jsonPayload: ReportedErrorEvent & { "@type": string } = {
            ...
            serviceContext: {
                "service": context.get("iopa.Path").replace(/^\//, '').replace(/\//g, '-'),
                "version": version,
    ...
    protected static async writeEntries(entries: LogEntry[]) {
        ...
        StackDriver.log_client.loggingEntriesWrite(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined,
            body,
            {})
        ...
        try {
            ...
            const result = await retry(async bail => {
                ...
                if (result.status == 403) {
                    ...
                    bail(new Error('Not Found ' + JSON.stringify(init)) + JSON.stringify(result))
                }
